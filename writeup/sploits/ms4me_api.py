...
class Client:
    ...
    def get_my_games(self) -> list[Game]:
        r = self.session.get(f"{self.http_url}/game/api/v1/user/game")
        data = r.json()
        if data["status"] == STATUS_ERROR:
            raise Exception(data["error"])
        return [Game(**i) for i in data["games"]]
    ...
    def run_common_ws_conn(self, on_message) -> websocket.WebSocketApp:
        def on_open(ws: websocket.WebSocketApp):
            ws.send_text(json.dumps({"token": self.token}))
